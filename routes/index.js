...
router.get('/:type/:blogid', function(req, res, next) {
	...
	blogDao.getBlogById(data,function(err,rows){
		if(err){
			...
			if(rows.length){
				messageDao.getMessageListByBlogId({blogid:data.id},function(err,lines){
					if(err){
						console.log(err);
						res.render('blogdetail.ejs', {title: 'MicroBlog System' ,userid:req.session.userid,blogDetail:rows[0],messageList:''});
					}else{
						console.log(lines);
						res.render('blogdetail.ejs', {
							title: 'MicroBlog System',
